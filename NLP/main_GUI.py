...
from BiLSTM_segment.models import predict
...
class SegmenterGUI(QWidget):
    ...
    def init_ui(self):
        ...
        self.llama_radio = QRadioButton('llama3.2分词')
        self.model_radio = QRadioButton('BiLSTM模型分词')
        ...
        self.algorithm_group.addButton(self.model_radio)
        ...
        algo_layout.addWidget(self.model_radio)
    ...
    def start_segmentation(self):
        ...
        if self.jieba_radio.isChecked():
            ...
            self.metrics_edit.setText('')  # 清空评估指标
            self.result_label.setText(f'分词结果：')
            text1 = f'请你帮我做一个分词的任务，用空格隔开每个词，只用给我回复结果,原文本：{text}'
            ...
            for chunk in stream:
                if not chunk['done']:
                    ...
                    self.result_edit.append(content)
            # content = ollama.generate(model='llama3.2', prompt=text1)  # 修改大模型名称2
            # content = str(content)
            # self.result_edit.setText(content)
            words = []
            ...
            start_time = time.time()
            # 使用并行双向最大匹配算法分词
            model_path = r'../BiLSTM_segment/models/bilstm_segmentation.pth'
            vocab_path = r'../BiLSTM_segment/data/vocab.txt'
            model, vocab = predict.load_model(model_path, vocab_path)
            words = predict.segment_sentence(model, vocab, text)
            end_time = time.time()
            # 评估分词结果
